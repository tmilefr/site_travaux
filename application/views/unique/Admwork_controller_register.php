 ...
 * Toggle cartes (v3) / liste compacte (v4)
 * Préférence persistée en cookie côté PHP (pas de localStorage)
 */

/* -----------------------------------------------------------------------
   PRÉFÉRENCE DE VUE (cookie, fallback = cartes)
   ----------------------------------------------------------------------- */
$pref_view = (isset($_COOKIE['aw_view']) && $_COOKIE['aw_view'] === 'list') ? 'list' : 'cards';
...
foreach ($works as $w) {
    if (strtotime($w->date_travaux) < $today_ts) {
        $past[] = $w;
    } else {
        $upcoming[] = $w;
...
/* Groupement par mois (commun aux deux vues) */
$upcoming_by_month = [];
...
function _v5_datefr($d,$j,$m){ $ts=strtotime($d); return $j[(int)date('w',$ts)].'\u00a0'.date('j',$ts).'\u00a0'.$m[(int)date('n',$ts)]; }
function _v5_stripe($t)    { return 'aw-stripe-'.(in_array($t,['TRA','MEN','INF','GOU','LAV','DEC','URG'])?$t:'default'); }
function _v5_badge($t)     { return 'aw-badge-'.(in_array($t,['TRA','MEN','INF','GOU','LAV','DEC','URG'])?$t:'default'); }
...
<div class="grid grid_12">
    <div class="aw-page-head">
        ...
        <div class="aw-view-toggle">
            <button class="aw-vt-btn<?php echo $pref_view==='cards'?' is-active':''; ?>"
                    onclick="awSetView('cards')" title="Vue cartes">
            ...
            <button class="aw-vt-btn<?php echo $pref_view==='list'?' is-active':''; ?>"
                    onclick="awSetView('list')" title="Vue liste">
...
<div class="grid grid_12"><div class="aw-root">
    ...
    <!-- ================================================================
         VUE CARTES (v3)
         ================================================================ -->
    <div id="aw-view-cards" class="aw-view<?php echo $pref_view==='cards'?'':' aw-view-hidden'; ?>">

        <!-- Section À venir -->
        <div class="aw-section-head upcoming">
            <span class="sh-dot"></span><h2>À venir</h2>
                    ...
                    <div class="aw-card-body">
                        <div class="aw-card-dateline">
                            <span class="aw-card-date"><?php
                                $ts=$ts=strtotime($w->date_travaux);
                                echo $jours_fr[(int)date('w',$ts)].' '.date('j',$ts).' '.$mois_abr[(int)date('n',$ts)];
                            ?></span>
                            <span class="aw-countdown <?php echo $cd_cls; ?>"><?php echo $cd; ?></span>
                        </div>
                        <p class="aw-card-title"><?php echo htmlspecialchars($w->titre); ?></p>
                        <span class="aw-type-badge <?php echo _v5_badge($w->type); ?>"><?php echo $bl; ?></span>
        ...
        <div class="aw-today-marker"><span class="tm-line"></span><i class="icon-calendar"></i>&nbsp;Aujourd'hui · <?php echo date('d/m/Y'); ?><span class="tm-line"></span></div>
        <?php endif; ?>

        <!-- Passées en cartes -->
        <div class="aw-section-head past">
            <span class="sh-dot"></span><h2>Sessions passées</h2>
        ...
        <?php $midx=0; foreach ($past_by_month as $mkey => $mdata): $first=($midx===0); ?>
            <div class="aw-month-header <?php echo $first?'is-open':''; ?>" onclick="awToggleMonth('cp-month-<?php echo $mkey; ?>',this)">
                <div class="aw-month-title"><i class="icon-calendar"></i><?php echo $mdata['label']; ?><span class="aw-month-badge"><?php echo count($mdata['works']); ?> session<?php echo count($mdata['works'])>1?'s':''; ?></span></div>
                <i class="aw-month-chevron icon-down-open"></i>
            </div>
            <div class="aw-month-body <?php echo $first?'is-open':''; ?>" id="cp-month-<?php echo $mkey; ?>">
                <div class="aw-cards-grid aw-cards-grid-past">
                <?php foreach ($mdata['works'] as $w):
                    ...
                    $bl   = isset($design->title)?$design->title:$w->type;
                ?>
                    <div class="aw-card is-past archived <?php echo $mine; ?>"
                         data-type="<?php echo $w->type; ?>"
                         data-title="<?php echo htmlspecialchars(strtolower($w->titre)); ?>"
                         data-ecole="<?php echo htmlspecialchars(strtolower($w->ecole)); ?>">
                        <div class="aw-card-inner <?php echo _v5_stripe($w->type); ?>">
                            <div class="aw-card-body">
                                <div class="aw-card-dateline">
                                    <span class="aw-card-date"><?php $ts=strtotime($w->date_travaux); echo $jours_fr[(int)date('w',$ts)].' '.date('j',$ts).' '.$mois_abr[(int)date('n',$ts)]; ?></span>
                                ...
                                <div class="aw-card-meta">
                                    <div class="aw-meta-row"><i class="icon-pin-outline"></i><?php echo $this->render_object->RenderElement('ecole',$w->ecole); ?></div>
                                    <?php if ($w->type_session==1): ?><div class="aw-meta-row"><i class="icon-clock-1"></i><?php echo $w->heure_deb_trav; ?> – <?php echo $w->heure_fin_trav; ?></div><?php endif; ?>
                                    <div class="aw-meta-row"><i class="icon-info-outline"></i><?php echo $this->render_object->RenderElement('type_session',$w->type_session); ?></div>
                                </div>
                            </div>
                            <div class="aw-card-footer">
                                <span class="aw-slots"><strong><?php echo (int)$w->participant; ?></strong>/<?php echo (int)$w->nb_inscrits_max; ?><?php if (!empty($w->already_registred)): ?>&nbsp;<span class="aw-badge-inscrit">&#10003;</span><?php endif; ?></span>
                                <div class="aw-footer-actions">
                                    <?php if ($acl==='sys'): ?><a href="<?php echo base_url('Admwork_controller/managed_one/'.$w->id); ?>" class="aw-btn adm"><?php echo $this->lang->line('ADM_WORK'); ?></a>
                                    <?php else: ?><a href="<?php echo base_url('Admwork_controller/register_one/'.$w->id); ?>" class="aw-btn details">Détails</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- ================================================================
         VUE LISTE (v4)
         ================================================================ -->
    <div id="aw-view-list" class="aw-view<?php echo $pref_view==='list'?'':' aw-view-hidden'; ?>">
        ...
        <?php if (empty($upcoming_by_month)): ?>
            <p class="aw-empty">Aucune session à venir pour le moment.</p>
        <?php else: ?>
        <div class="aw-list" id="aw-list-upcoming">
            ...
            <div class="aw-list-row <?php echo $mine.' '.$dispo; ?>"
                ...
                <div class="aw-list-info">
                    ...
                    <div class="aw-list-meta">
                        ...
                        <?php if ($w->type_session==1): ?><span class="aw-list-dot">·</span><span><?php echo $w->heure_deb_trav; ?>–<?php echo $w->heure_fin_trav; ?></span><?php endif; ?>
                        <span class="aw-list-dot">·</span>
                        <span class="aw-list-badge <?php echo _v5_badge($w->type); ?>"><?php echo $bl; ?></span>
                    </div>
                </div>
                ...
                <div class="aw-list-action">
                    ...
                    <?php else: ?><span class="aw-btn complet">Complet</span><?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
        <?php endforeach; ?>
        ...
        <div class="aw-accordion-past">
        <?php $midx=0; foreach ($past_by_month as $mkey => $mdata): $first=($midx===0); ?>
            <div class="aw-month-header <?php echo $first?'is-open':''; ?>" onclick="awToggleMonth('lp-month-<?php echo $mkey; ?>',this)">
                <div class="aw-month-title"><i class="icon-calendar"></i><?php echo $mdata['label']; ?><span class="aw-month-badge"><?php echo count($mdata['works']); ?> session<?php echo count($mdata['works'])>1?'s':''; ?></span></div>
                <i class="aw-month-chevron icon-down-open"></i>
            </div>
            <div class="aw-month-body <?php echo $first?'is-open':''; ?>" id="lp-month-<?php echo $mkey; ?>">
            <?php foreach ($mdata['works'] as $w):
                $design = $this->render_object->GetDesign($w->type);
                list($cd,$cd_cls) = _v5_countdown($w->date_travaux);
                $mine = !empty($w->already_registred)?'mine':'';
                ...
                <div class="aw-list-row is-past archived <?php echo $mine; ?>"
                    ...
                    <div class="aw-list-info">
                        ...
                        <div class="aw-list-meta">
                            ...
                            <?php if ($w->type_session==1): ?><span class="aw-list-dot">·</span><span><?php echo $w->heure_deb_trav; ?>–<?php echo $w->heure_fin_trav; ?></span><?php endif; ?>
                            <span class="aw-list-dot">·</span>
                            <?php if (!empty($w->already_registred)): ?><span class="aw-badge-inscrit">&#10003; participé</span>
                            <?php else: ?><span class="aw-list-not-reg">non inscrit</span><?php endif; ?>
                        </div>
                    </div>
                    <div class="aw-list-slots"><span class="aw-slots-num"><strong><?php echo (int)$w->participant; ?></strong>/<?php echo (int)$w->nb_inscrits_max; ?></span></div>
